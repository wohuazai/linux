		...
		-   struct tasklet_struct
		    {
		        ...
		        unsigned long state;
		        atomic_t count;
		        void (*func)(unsigned long);  //中断下半部的要执行的方法
		        unsigned long data;
		    };

		-  初始化tasklet（硬件初始化时），为下半部做准备 
			tasklet_init(&key_dev->mytasklet,tasklet_half_irq_handle, 34);
		
		- 在中断执行函数 （中断函数中）
		  执行   tasklet_schedule(&key_dev->mytasklet) （启动下半部）；（相当于创建线程）
		
		- 将比较费时间的部分放到 tasklet_half_irq_handle(unsigned long data)（中断下半部函数）执行。
